...
FUNCTION(CHECK_PYTHON_REQS module_list test_name add_test)
  ...
  GET_PROPERTY(local_found GLOBAL PROPERTY QMC_PYTHON_MODULES_FOUND)
  GET_PROPERTY(local_missing GLOBAL PROPERTY QMC_PYTHON_MODULES_MISSING)
  foreach(python_module IN LISTS ${module_list})
    list(FIND local_found "${python_module}" cached_python_module_found)
    list(FIND local_missing "${python_module}" cached_python_module_missing)
    if(cached_python_module_found GREATER -1)
      continue()
    elseif(cached_python_module_missing GREATER -1)
      ...
      if (NOT(has_python_module))
        list(APPEND local_missing ${python_module})
        if (test_name)
          MESSAGE("Missing python module ${python_module}, not adding test ${test_name}")
        endif()
        set(${add_test} false PARENT_SCOPE)
        ...
        list(APPEND local_found PARENT_SCOPE ${python_module})
      endif()
    endif()
  endforeach()
  SET_PROPERTY(GLOBAL PROPERTY QMC_PYTHON_MODULES_FOUND "${local_found}")
  SET_PROPERTY(GLOBAL PROPERTY QMC_PYTHON_MODULES_MISSING "${local_missing}")
ENDFUNCTION()
